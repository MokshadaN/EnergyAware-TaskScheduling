"""Cloud Environment simulation engine for energy-aware task scheduling.

Implements realistic task workload generation, heterogeneous virtual machine clusters,
energy calculation models, and multi-objective metrics tracking.
"""
...
from core.models import Resource, Task


class CloudEnvironment:
    """Heterogeneous Cloud Computing Environment for reinforcement-learning based task allocation."""
    ...
    def _initialize_tasks_realistic(self) -> List[Task]:
        ...
        for i in range(self.num_tasks):
            ...
            deps = random.sample(range(max(0, i - 10), i), min(5, i)) if i > 0 and random.random() < 0.3 else []
            task = Task(
                task_id=i,
                r_i=random.uniform(*pattern["r_range"]),
                d_i=random.uniform(*pattern["d_range"]),
                e_i=random.uniform(*pattern["e_range"]),
                p_i=float(np.random.exponential(scale=0.5)),
                dep_i=deps,
            )
            tasks.append(task)
    ...
    def _initialize_resources_heterogeneous(self) -> List[Resource]:
        ...
        for i in range(self.num_vms):
            ...
            resources.append(Resource(vm_id=i, c_j=capacity, eta_j=energy_rate, u_j=0.0))
        return resources

    def get_state(self) -> np.ndarray:
        uptime = min(100.0, (self.current_time / max(1, self.num_tasks)) * 100.0)
        ...
        return np.array([uptime, cpu_util, memory_util, disk_util, ram_util, load_balance], dtype=np.float32)
    ...
    def _get_cpu_utilization(self) -> float:
        ...
        return float(total_util * 100.0)

    def _get_memory_utilization(self) -> float:
        base_util = 20.0 + 60.0 * (self.current_task_idx / max(1, self.num_tasks))
        fragmentation = 5.0 * np.sin(self.current_task_idx / 10.0)
        return float(min(95.0, max(10.0, base_util + fragmentation)))

    def _get_disk_utilization(self) -> float:
        io_intensity = 0.3 * (self.current_task_idx / max(1, self.num_tasks))
        base_util = 10.0 + 50.0 * (self.current_task_idx / max(1, self.num_tasks)) + io_intensity
        return float(min(85.0, base_util))

    def _get_load_balance(self) -> float:
        if not self.resources:
            return 1.0
        utilizations = [r.u_j for r in self.resources]
        variance = float(np.var(utilizations)) if len(utilizations) > 1 else 0.0
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        contention_factor = 1.0 + selected_vm.u_j * 0.5
        execution_time = base_exec_time * contention_factor

        base_energy = selected_vm.eta_j * current_task.r_i * current_task.d_i / 3600.0
        energy_scaling = 1.0 + 0.2 * selected_vm.u_j
        ...
        selected_vm.u_j = float(min(1.0, selected_vm.u_j * 0.95 + utilization_increment))
        ...
        self.metrics_history["deadline_misses"].append(0.0 if deadline_met else 1.0)
        ...
        energy_reward = -task_energy * 10.0
        deadline_reward = 10.0 if deadline_met else -20.0
        utilization_reward = -abs(selected_vm.u_j - 0.7) * 5.0
        load_balance_reward = self._get_load_balance() * 5.0
    ...
    def get_metrics(self) -> Dict[str, float]:
        return {
            ...
            "avg_response_time": float(np.mean(self.metrics_history["response_times"])) if self.metrics_history["response_times"] else 0.0,
            "throughput": float(self.metrics_history["throughput"][-1]) if self.metrics_history["throughput"] else 0.0,
            "task_completion_rate": float((self.current_task_idx / max(1, self.num_tasks)) * 100.0),
            "deadline_met_rate": float(1.0 - np.mean(self.metrics_history["deadline_misses"])) if self.metrics_history["deadline_misses"] else 1.0,
            "avg_load_balance": float(np.mean(self.metrics_history["load_balance"])) if self.metrics_history["load_balance"] else 0.0,
            "energy_efficiency": float(self.current_task_idx / max(1e-5, self.total_energy)) if self.total_energy > 0 else 0.0,
        }
